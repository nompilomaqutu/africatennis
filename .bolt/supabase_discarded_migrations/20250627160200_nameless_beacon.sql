/*
  ...
  1. Changes
    - Update the generate_tournament_bracket function to work with any number of participants (minimum 2)
    - Improve error handling and validation
    - Fix bracket generation for tournaments with fewer than maximum participants
    - Add better participant count validation

  2. Security
    - Maintain existing RLS policies
*/

-- Update the generate_tournament_bracket function to work with any number of participants
  ...
  CASE tournament_format
    WHEN 'single_elimination' THEN
      ...
      DECLARE
        ...
        num_first_round_matches INTEGER;
        num_byes INTEGER;
        seeded_positions INTEGER[];
        match_number INTEGER := 1;
      BEGIN
        -- Calculate the number of first-round matches needed
        -- For non-power-of-2 participant counts, we need to handle byes
        num_first_round_matches := GREATEST(num_participants / 2, 1);
...
BEGIN
  ...
  IF participant_count >= max_participants AND tournament_status = 'registration_open' THEN
    ...
    BEGIN
      ...
      IF NOT (bracket_result->>'success')::BOOLEAN THEN
        RAISE NOTICE 'Error generating bracket: %', bracket_result->>'error';
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE NOTICE 'Error generating bracket: %', SQLERRM;
    END;
  END IF;
