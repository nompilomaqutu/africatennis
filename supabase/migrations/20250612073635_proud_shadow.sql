/*
  ...
  1. Changes
    - Alter matches.score column from TEXT to JSONB with default structure
    - Update match_events.event_type constraint to use tennis-specific event types
    - Add new function to handle tennis scoring logic

  2. Security
    - Maintain existing RLS policies
*/

-- Alter matches table to use JSONB for score
ALTER TABLE matches 
  ALTER COLUMN score TYPE JSONB USING 
    CASE 
      WHEN score IS NULL THEN 
        json_build_object(
          'sets', json_build_array(),
          'current_game', json_build_object('player1', '0', 'player2', '0'),
...
-- Set default value for score column
ALTER TABLE matches 
  ALTER COLUMN score SET DEFAULT json_build_object(
    'sets', json_build_array(),
    'current_game', json_build_object('player1', '0', 'player2', '0'),
    'server_id', NULL,
    'is_tiebreak', false
...
-- Drop existing constraint on match_events.event_type
ALTER TABLE match_events DROP CONSTRAINT IF EXISTS match_events_event_type_check;

-- Add new constraint for tennis-specific event types
...
-- Create a function to calculate tennis score
