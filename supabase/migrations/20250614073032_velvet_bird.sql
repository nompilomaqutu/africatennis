/*
  ...
  1. Changes
    - Update the calculate_tennis_score function to properly handle tennis scoring
    - Fix issues with score initialization and progression
    - Ensure proper server tracking and switching
    - Add support for tiebreaks
...
CREATE OR REPLACE FUNCTION calculate_tennis_score(
  ...
  point_type TEXT DEFAULT 'point_won'
) RETURNS JSONB AS $$
DECLARE
  match_record RECORD;
  current_score JSONB;
  ...
  IF game_won THEN
    ...
    BEGIN
      ...
      new_game := jsonb_build_object(
        'player1_points', player1_is_winner::INT,
        'player2_points', player2_is_winner::INT,
        'server_id', server_id
      );
