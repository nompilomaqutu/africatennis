/*
  # Fix Tournament Bracket Generation for Manually Closed Registration

  1. Changes
    - Drop and recreate functions with updated return types and logic
    - Improve error handling and validation for tournament bracket generation
    - Fix handling of tournaments with fewer than maximum participants

  2. Security
    - Maintain existing RLS policies
*/

-- First drop the existing functions to avoid the return type error
DROP FUNCTION IF EXISTS public.generate_tournament_bracket(uuid);
DROP FUNCTION IF EXISTS public.manually_start_tournament(uuid);
DROP FUNCTION IF EXISTS public.check_tournament_full_and_generate_bracket();

-- Recreate the generate_tournament_bracket function with JSONB return type
...
-- Recreate the manually_start_tournament function
...
-- Recreate the check_tournament_full_and_generate_bracket function
...
-- Recreate the tournament_full_check trigger
DROP TRIGGER IF EXISTS tournament_full_check ON tournament_participants;
